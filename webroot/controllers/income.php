    ...
    class Income extends CI_Controller {
        ...
        public function insert_record() {
            ...
            if ($this->form_validation->run() == FALSE) {       //if it hasn't submitted or there are erros
                ...
                if ($this->Income_model->insert_income($cheynId)) {       //If we successfully inserted entry
                    $this->session->set_tempdata('income_entry_success', 'Entry saved successfully', 5);
                    redirect('income/index');
                } else {
                    $this->session->set_tempdata('income_entry_failure', 'Couldn\'t save entry! Please try again', 5);
                    redirect('income/index');
                }
        ...
        public function get_income() {
            ...
            $config["base_url"] = base_url() . "/income/get_income";         //set the link to the method
            $config["total_rows"] = $this->Income_model->record_count($cheynId);    //get total number of income records
            ...
            $data["incomes"] = $this->Income_model->get_spec_income($config["per_page"], $page, $cheynId);
            //die(print_r($data['incomes']));
            ...
            //Get the income proportions of the user's buckets
            $data['centiles'] = $this->Charts_model->bucket_proportions($cheynId, "income");

            //Get the income proportions of the user's buckets
            $data['monthlies'] = $this->Charts_model->monthlies($cheynId, "income");
        ...
        public function view($id) {
            ...
            $data['content'] = $this->Income_model->get_income($cheynId, $id);
        ...
        public function delete($id) {
            ...
            $data['reply'] = $this->Income_model->delete_income($cheynId, $id);
        ...
        public function update($id) {
            ...
            if ($this->form_validation->run() == FALSE) {       //if it hasn't submitted or there are erros
                ...
                //Try inserting the new income entry for this user
                if ($this->Income_model->update_income($cheynId, $id)) {       //If we successfully inserted entry
                    $this->session->set_flashdata('update_entry_success', 'Entry updated successfully');
                    redirect('income/view/' . $id);
                } else {
