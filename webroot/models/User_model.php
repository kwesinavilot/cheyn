<?php
    ...
    class User_model extends CI_Model {
        ...
        public function login($email, $password) {
            ...
            //Check for the email
            $this->db->select('users.firstname, login.cheynID, login.email, password');      //Select id, email and password
        ...
        public function getUserDetails($cheynId) {
            //die(print_r($this->input->post()));
            ...
            if($query->num_rows() == 1) {
                ...
                $this->cheynId = $resultSet->cheynID;
        ...
        public function getDashes($cheynId) {
            // //Check for the email
            ...
            // $this->db->join('expenses', 'income.cheynID = expenses.cheynID', 'inner');     //inner join expenses on 

            // $where = "income.cheynID = '$cheynId' AND expenses.cheynID = '$cheynId'";           //Create custom where clause
            // $this->db->where($where);      //Create where clause using custom where clause
            // $query = $this->db->get();
        ...
        public function update_details($cheynId){
            $data = array(
                'firstname' => $this->input->post('firstname'),
            ...
            $this->db->trans_start();                                   //Start the transaction
                ...
                //Update the users table with details based on cheynId
                $this->db->where('CheynID', $cheynId);
                $this->db->update('users', $data);

                //Update the email of the user in the login table
                // UPDATE `login` SET `email` = 'Entered email' WHERE `CheynId` = current user's CheynId
        ...
        public function change_password($cheynId){
            ...
            // UPDATE `login` SET `password` = 'new_password' WHERE `CheynId` = current user's CheynId
        ...
        public function verify_password($cheynId) {
            ...
            //Verify CheynID and get the user's password
